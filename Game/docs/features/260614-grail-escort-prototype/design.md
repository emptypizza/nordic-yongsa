...
- 캐릭터는 `glbs/` 복셀 메시, 배경/소품은 프리미티브/빌보드 + 텍스처(점진 교체 중):
  - **2026-06-27 prop 빌보드 패스**: 나무·통나무·성배마차·포탈 4종을 생성형 2D 아트로 교체.
    ...
    `CharacterMesh.build_billboard(tex, world_size, fit)`가 카메라-페이싱 `Sprite3D`(billboard·unshaded·
    `ALPHA_CUT_OPAQUE_PREPASS`·밑면 바닥정렬)를 생성. **텍스처가 없으면 각 prop의 기존 프리미티브로 폴백**(헤드리스/누락 안전).
