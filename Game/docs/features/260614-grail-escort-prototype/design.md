...
- **적 ↔ 용사**: 근접 + 적이 기절 상태가 아니면 → 용사와 적이 서로 반대 방향으로 넉백. 적은 2칸 넉백 + 1초 기절, 용사는 1칸 넉백(기절 없음).
...
- 적 스폰: 성배에서 맨해튼 거리 8칸 이상 떨어진 그리드 레인에 스냅해 생성. 동시 적 수는 `GameManager`가 제한하고, 기본 스폰 간격은 2.5초로 둔다.
