...
> [!warning] 이 문서는 더 이상 유효하지 않음 (SUPERSEDED)
> 이 계획서는 **C#(godot-mono) 구현 계획**이며, **GDScript 마이그레이션(2026-06-26)으로 대체(superseded)되었다.**
> 아래에 적힌 게임 상수는 더 이상 현재 코드/설계와 일치하지 않는다:
> - 그리드: 7×12(이 계획) → **19×30(현재)**
> - 적 스폰 최소 거리: 4 → **10**
> - 스폰 간격: 1.2s → **2.5s**
> - 성배 이동: 칸 단위 auto-hop(이 계획) → **연속 전진(continuous advance, 현재)**
>
> 현재 동작·구조의 **단일 진실(single source of truth)은 같은 폴더의 `design.md`**다.
> 이 계획서는 당시 C# 구현 이력으로만 참고한다.
